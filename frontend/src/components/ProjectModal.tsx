...
import { Project, UserWithStats, STATUS_OPTIONS, BOT_SUB_STATUSES, BOT_COLORS, BotColor } from "@/lib/types";
...
export function ProjectModal({ open, onClose, onSaved, project, users }: {
  open: boolean;
  onClose: () => void;
  onSaved: () => void;
  ...
  useEffect(() => {
    ...
    if (project) {
      setForm({
        ...
        bot_comment: project.bot_comment ?? "",
  ...
  async function save() {
    ...
    try {
      ...
      onClose(); onSaved(); toast(project ? "Проект обновлён" : "Проект добавлен");
  ...
  return (
    <Modal open={open} onClose={onClose} title={project ? "Изменить проект" : "Добавить проект"} width={460}
      footer={
        <>
